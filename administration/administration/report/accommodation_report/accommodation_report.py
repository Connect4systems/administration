...
def execute(filters=None):
	...
	employees = frappe.get_list(
		"Employee", filters={"name": ["in", employee_ids]}, fields=["name", "employee_name", "designation"],
		limit_page_length=0,
...
def build_rows(flats, rooms, beds, employees, filters):
	...
	employee_names = {employee["name"]: employee.get("employee_name") or "" for employee in employees}
	for room in rooms:
...
def get_columns():
	return [
		...
		for field, label, target, width in (
			...
			("employee", "Employee ID", "Employee", 260),
